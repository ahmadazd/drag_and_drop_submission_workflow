...
parser.add_argument('-o', '--output',
                    help='Specify output directory to transfer files from S3 bucket to, and hold files + subdirs generated by the script',
                    type=str, required=False)  # False #No trailing slash
...
def transfer_check(file):
    with open(f'{file}', 'r') as stdout:
        ...
        print(f"files transferred to {main.outdir} =", total_transfer)  # TODO: remember if the script is re-run, this statement will not be accurate as passed files will be deleted
...
def main():
    ...
    if args.output:
        main.transfer_cmds = f"cd {main.outdir} && aws --endpoint-url https://uk1s3.embassy.ebi.ac.uk s3 cp s3://covid-utils-ui-88560523/{args.UUID} . --recursive"
    else:
        main.transfer_cmds = f"cd {os.path.dirname(main.outdir)}; cd venv/; source bin/activate && cd {main.outdir} && aws --endpoint-url https://uk1s3.embassy.ebi.ac.uk s3 cp s3://covid-utils-ui-88560523/{args.UUID} . --recursive" # default dir on codon
    ...
    for main.fname in os.listdir(main.outdir):
        if any(fnmatch.fnmatch(main.fname, ext) for ext in file_exts):
            ...
            new_df = pd.DataFrame([{file_name: new_filename, uploaded_md5_val: original_md5, new_md5_val: new_md5, integrity_check: "PASS" if new_md5 == original_md5 else "FAIL"}])
    ...
    # Create all subdirs and move files to appropriate subdir
    for dir in subdirs:
        setup_subdirs(dir)

    # error check that count of files in pass + fail dir = total in summary_df
    try:
        len(os.listdir(f'{main.outdir}/{main.trans_int_outdir}/pass')) + len(os.listdir(f'{main.outdir}/{main.trans_int_outdir}/fail')) == len(main.summary_df)
    except:
